...
>(
    ...
    descriptor.value = async function (this: LevelPstCache, ...args: TParams) {
        logger.log(`[PstCache][SoftLockDb][${_property}] before open`);
        await this.db.open();
        logger.log(`[PstCache][SoftLockDb][${_property}] after open`, this.db);
        const ret = await (originalMethod as AnyFunction).apply(this, args);
        logger.log(`[PstCache][SoftLockDb][${_property}] after exec`, {
            ret,
        });
        await this.db.close();
        logger.log(`[PstCache][SoftLockDb][${_property}] after close`, this.db);
        return ret;
    } as TMeth;
};
...
export class LevelPstCache extends AbstractPstCache {
    ...
    constructor(cachePath?: string) {
        super(cachePath);
        logger.log("[PstCache] constructor", { cachePath });
        this.db = new Level(
            path.resolve(this.cachePath, CACHE_FOLDER_NAME),
            defaultDbOptions
        );
        logger.log("[PstCache] DB", {
            db: this.db,
