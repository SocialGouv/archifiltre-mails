...
export const IS_MAIN: boolean = ipcMain && !ipcRenderer;
...
/**
 * Isomorphic service handling user config. Use `electron-store` under the hood on main side.
 *
 * Renderer processes can remotly access access to this config. But saves have to be done from main process.
 */
class UserConfigService implements Service {
...
/**
 * Exposed instance of the user config. `init()` **must** be called once before usage.
 */
